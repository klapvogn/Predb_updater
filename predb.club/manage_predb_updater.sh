#!/bin/bash
# PreDB.club Updater - Tmux Management Script
...
start_updater() {
    if session_exists; then
        ...
        echo "Use './manage_predb_updater.sh status' to check status"
        return 1
    fi
    
    echo -e "${GREEN}Starting PreDB.club Updater...${NC}"
    ...
    # Rename the window
    tmux rename-window -t $TMUX_SESSION:0 'PreDBClubUpdater'
    
    # Start the updater
    tmux send-keys -t $TMUX_SESSION:0 "cd $SCRIPT_DIR" C-m
    tmux send-keys -t $TMUX_SESSION:0 "clear" C-m
    tmux send-keys -t $TMUX_SESSION:0 "echo '=== PreDB.club Updater Started at $(date) ==='" C-m
    tmux send-keys -t $TMUX_SESSION:0 "php $PHP_SCRIPT" C-m
    
    sleep 2
    
    if session_exists; then
        echo -e "${GREEN}✓ PreDB.club Updater started successfully${NC}"
        echo ""
        echo "Commands:"
        echo "  Attach to session:  ./manage_predb_updater.sh attach"
        echo "  View logs:         ./manage_predb_updater.sh logs"
        echo "  Check status:      ./manage_predb_updater.sh status"
        echo "  Stop updater:      ./manage_predb_updater.sh stop"
    else
...
stop_updater() {
    ...
    echo -e "${YELLOW}Stopping PreDB.club Updater...${NC}"
    ...
    if ! session_exists; then
        echo -e "${GREEN}✓ PreDB.club Updater stopped${NC}"
    else
...
restart_updater() {
    echo -e "${YELLOW}Restarting PreDB.club Updater...${NC}"
    stop_updater
    sleep 2
    start_updater
}
...
show_status() {
    if session_exists; then
        echo -e "${GREEN}● PreDB.club Updater is RUNNING${NC}"
        echo ""
        ...
        echo ""
        echo "To attach: ./manage_predb_updater.sh attach"
    else
        echo -e "${RED}○ PreDB.club Updater is NOT RUNNING${NC}"
        echo ""
        echo "To start: ./manage_predb_updater.sh start"
    fi
}
...
attach_session() {
    if ! session_exists; then
        ...
        echo "Start it first with: ./manage_predb_updater.sh start"
        return 1
    fi
...
show_help() {
    echo "PreDB.club Updater - Tmux Management Script"
    echo ""
    echo "Usage: ./manage_predb_updater.sh [COMMAND]"
    echo ""
    echo "Commands:"
    ...
    echo "  ./manage_predb_updater.sh status"
    echo "  ./manage_predb_updater.sh logs"
    echo ""
